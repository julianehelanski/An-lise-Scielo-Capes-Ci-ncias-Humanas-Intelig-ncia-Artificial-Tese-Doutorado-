...
%   - As figuras NÃO trazem título embutido na imagem: a identificação é feita
%     pelo nome do arquivo e pela \caption{}. As ressalvas analíticas que antes
%     apareciam no título (p. ex. "2024 subindexado", "linhas tracejadas =
%     medianas", "escala limitada a 10%+") estão incorporadas às legendas abaixo.
%   - Toda figura é gerada em dois formatos de mesmo nome-base: .png (raster,
%     300 dpi, usado no \includegraphics) e .svg (vetorial). Troque a extensão
%     se preferir o SVG (requer suporte a SVG no fluxo LaTeX).
...
\begin{figure}[htbp]
  ...
  logarítmico) e de taxa interna (eixo vertical) --- marcadas pelas linhas
  tracejadas --- entre os quarenta maiores produtores de IA nas Humanidades,
  2016--2024. Os quatro quadrantes distinguem países líderes, de nicho,
  ``gigantes latentes'' e periféricos. O Brasil, destacado em vermelho,
  posiciona-se entre os ``gigantes latentes'': alto volume, baixa penetração.}
